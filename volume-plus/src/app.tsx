...
async function main() {
	...
	function stepVolume(step: number) {
		if (debounce) {
			return;
		}
		debounce = true;

		const newVolume = clamp(PlaybackAPI._volume + step, 0, 1);
	...
	container.addEventListener("wheel", (ev) => {
		...
		stepVolume(-Math.sign(ev.deltaY) * step);
	});
	...
	document.addEventListener("keydown", (ev) => {
		if (
			...
			if (ev.key == "ArrowLeft" || ev.key == "ArrowRight") {
				field = Number(settings.getFieldValue("default-increment"));
				if (ev.key == "ArrowLeft") {
					field = -field;
				}
			} else if (ev.key == "ArrowUp" || ev.key == "ArrowDown") {
				field = Number(settings.getFieldValue("shift-increment"));
				if (ev.key == "ArrowDown") {
					field = -field;
				}
			}
