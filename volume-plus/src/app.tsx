...
async function main() {
	...
	function createInput(nameId: string, description: string, defaultValue: string) {
		if (!settings.getFieldValue(nameId)) {
			settings.setFieldValue(nameId, defaultValue)
		}
		settings.addInput(nameId, description, defaultValue, undefined, undefined, {
			type: "number",
			min: 1,
			max: 100,
	...
	createInput("shift-increment", "Mouse wheel increment while holding Shift", "10");
	createInput("ctrl-increment", "Mouse wheel increment while holding Ctrl", "1");

	// render settings menu using spcr-settings plugin
	new Menu.Item("Volume+", false, () => {
